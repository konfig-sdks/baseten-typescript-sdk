...
import { readableStreamToString, BasetenError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BasetenError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BasetenError(e, e.response?.data)
                }
                if (innerError instanceof BasetenError) {
                    // Got "BasetenError" from the above try block
                    throw innerError;
                }
